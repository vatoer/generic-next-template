...
export default function ProfilesPage() {
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              <Avatar className="h-12 w-12 flex-shrink-0">
                <AvatarImage
                  src={profiles.find(p => p.id === selectedProfileId)?.avatar}
                  alt=""
                />
                <AvatarFallback className="text-lg font-semibold">
                  {getInitials(
                    profiles.find(p => p.id === selectedProfileId)?.nama || ""
                  )}
                </AvatarFallback>
              </Avatar>
          ...
          {profiles.map((profile: ProfilDTO) => (
            ...
            >
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <CardTitle className="text-lg">{profile.nama}</CardTitle>
                    <CardDescription className="mt-1">
                      {profile.deskripsi || "Tanpa deskripsi"}
                    </CardDescription>
                  </div>
                  {selectedProfileId === profile.id && (
              ...
              <CardContent className="space-y-3">
                ...
                {/* Organisasi Info */}
                {profile.organisasiId && (
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-muted-foreground">
                      Organisasi:
                    </span>
                    <span className="text-sm">{profile.organisasi?.nama}</span>
                  </div>
                )}
      ...
      <Card className="bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
        <CardHeader>
          <CardTitle className="text-base">Tentang Profil</CardTitle>
        </CardHeader>
        <CardContent className="text-sm space-y-2 text-blue-900 dark:text-blue-100">
          <p>
            Profil memungkinkan Anda untuk mengelola multiple konteks organisasi.
            Setiap profil dapat memiliki:
          </p>
          <ul className="list-disc list-inside space-y-1 ml-2">
            <li>Role dan permission berbeda</li>
            <li>Konteks organisasi yang berbeda</li>
            <li>Dashboard dan setting yang disesuaikan</li>
          </ul>
          <p className="pt-2">
            Gunakan tombol "Gunakan Profil" untuk beralih antar profil.
          </p>
        </CardContent>
      </Card>
    </div>
